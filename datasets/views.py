...
def link_uri(auth, id):
    ...
    if auth == 'align_match_data':
        ...
        uri = str(place.dataset.id) + ":" + str(place.id)
    else:
    ...
    for hit in raw_hits:
        ...
        if other_place_hit is not None:
            ...
            if other_place.geoms.first():
                # print("for hit with id {} found place with id {} and geom: {}".format(hit.id, other_place.id,json.dumps(other_place.geoms.first().jsonb)))
                geom = other_place.geoms.first().jsonb
                geom['ds'] = other_place.dataset.label
                hit_supplemental[hit.id]['geom'] = json.dumps(geom)
        else:
        ...
        if task.ready():
            ...
            if isinstance(task.result, Exception):
                ...
                if status_code == 400:
                    ...
                    try:
                        d1 = DatasetFile.objects.get(dataset_id=pk)
                        d2 = DatasetFile.objects.get(dataset_id=dt_2)
                    except Exception as e:
                        print("Exception in ds_recon when trying to fetch dataset files for {} and {}".format(pk,
                                                                                                              dt_2))
                        raise e
