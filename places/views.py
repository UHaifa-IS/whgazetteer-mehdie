...
def defer_review(request, pid, auth, last, tid=None):
    ...
    if auth in ['whg', 'idx']:
        ...
        p.review_wd = 2
    else:  # matched with another dataset
        d = p.dataset.id
        Hit.objects.filter(place_id=pid, dataset_id=d, task_id=tid).update(flag=True, reviewed=True)
...
class PlacePortalView(DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['core'] = ['gn500', 'gnmore', 'ne_countries', 'ne_rivers982', 'ne_mountains', 'wri_lakes',
                           'tgn_filtered_01']
